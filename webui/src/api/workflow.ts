...
async function cloneWorkflowWithFallback(
  id: string,
  data: {
    name?: string;
    description?: string;
  ...
  try {
    return await client.post<Workflow>(`/api/workflow/${id}/clone`, data);
  } catch (error: any) {
    if (error?.response?.status !== 404) {
      throw error;
    }

    // Backward compatibility for a running backend that has not loaded the
    // clone endpoint yet: export the source workflow and create a custom copy.
    const sourceResponse = await client.get<Workflow>(`/api/workflow/${id}`);
    const exportResponse = await client.get<WorkflowJSON>(`/api/workflow/${id}/export`);
    const source = sourceResponse.data;
    const now = Date.now();
    const newName = data.name || `${source.name} Copy`;
    const workflowJson: WorkflowJSON = {
      ...exportResponse.data,
      name: newName,
      metadata: {
        ...(exportResponse.data.metadata ?? {}),
        sourceWorkflowId: id,
    ...
    const createResponse = await client.post<Workflow>('/api/workflow', {
      name: newName,
      description: data.description ?? source.description,
      category: source.category,
      workflowJson,
      createdBy: data.createdBy ?? source.createdBy,
      source: 'global',
    });

    const targetStatus = data.status ?? source.status;
    if (targetStatus && createResponse.data.status !== targetStatus) {
      return await client.put<Workflow>(`/api/workflow/${createResponse.data.id}`, {
        status: targetStatus,
      });
    }
...
export const workflowAPI = {
  ...
  clone: cloneWorkflowWithFallback,
