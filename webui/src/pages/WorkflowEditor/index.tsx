...
export default function WorkflowEditor() {
  ...
  const isReadOnly = workflow?.source === 'project';
  ...
  }, [isReadOnly, nodes, edges, setNodes, setEdges]);
  ...
  const handleSave = async () => {
    if (!workflow) return;
    if (isReadOnly) {
      alert(t('editor.readonlyBuiltin'));
      return;
    }
  ...
  const handleClone = async () => {
    ...
    try {
      setCloning(true);
      const response = await workflowAPI.clone(workflow.id, {
        name: t('detail.clone.defaultName', { name: workflow.name }),
        description: workflow.description,
        status: workflow.status,
      });
      navigate(`/workflows/${response.data.id}/edit`);
    } catch (error: any) {
      alert(t('detail.clone.failed') + ': ' + (error.message || t('editor.unknownError')));
    } finally {
      setCloning(false);
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {isReadOnly && (
              <p className="mt-1 text-xs text-blue-600">{t('editor.readonlyBuiltin')}</p>
            )}
          </div>
          {validationResult && (
        ...
        <div className="flex items-center gap-3">
          ...
          {isReadOnly && (
            <button
              onClick={handleClone}
              disabled={cloning}
              className="flex items-center gap-2 px-4 py-2 text-blue-700 bg-white border border-blue-200 rounded-lg hover:bg-blue-50 transition-colors disabled:opacity-50"
            >
              <Copy className="w-4 h-4" />
              {cloning ? t('detail.rightPanel.cloning') : t('editor.cloneAndEdit')}
            </button>
          )}
          <button
      ...
      <div className="flex-1 relative">
        {/* 节点工具栏 */}
        {showNodeToolbar && !isReadOnly && <NodeToolbar onAddNode={handleAddNode} />}
