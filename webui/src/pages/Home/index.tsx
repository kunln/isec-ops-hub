...
function resolveHomeProductName(brandingName: string | undefined | null, fallback: string): string {
  const value = brandingName?.trim();
  if (!value || /^flocks(ai)?$/i.test(value)) return fallback;
  return value;
}
...
export default function Home() {
  ...
  const productName = resolveHomeProductName(branding.product_name, t('productName'));
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-sm text-red-600 ml-2">{t('stats.backendErrorHint')}</span>
              </div>
            </div>
          )}
