...
export default function WorkflowDetail() {
  ...
  const handleClone = useCallback(async () => {
    if (!workflow) return;
    try {
      const res = await workflowAPI.clone(workflow.id, {
        name: t('detail.clone.defaultName', { name: workflow.name }),
        description: workflow.description,
        status: workflow.status,
      });
      showToast('success', t('detail.clone.success'));
      navigate(`/workflows/${res.data.id}/edit`);
    } catch (err: unknown) {
      showToast('error', `${t('detail.clone.failed')}: ${extractErrorMessage(err)}`);
    }
  }, [workflow, navigate, showToast, t]);
    ...
    <div className="flex flex-col h-full bg-gray-50 overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        {drawerNode && (
          <>
            ...
            <NodeInfoPanel
              ...
              readOnly={workflow.source === 'project'}
            />
          </>
        )}
        ...
        <RightPanel
          ...
          onDelete={workflow.source === 'project' ? undefined : handleDelete}
          onClone={workflow.source === 'project' ? handleClone : undefined}
        />
      </div>
    </div>
  );
