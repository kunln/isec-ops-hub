...
interface UpdateModalProps {
  ...
  updateCheckAllowed?: boolean;
  updateApplyAllowed?: boolean;
  onClose: () => void;
  onDismiss?: () => void;
}
...
export default function UpdateModal({
  ...
  const fetchVersion = useCallback(async () => {
    if (!updateCheckAllowed) {
      setInfo(null);
      setError(t('checkDisabled', { defaultValue: 'Update checks are disabled by policy.' }));
      return;
    }
    setChecking(true);
    setError(null);
  ...
  }, [i18n.language, t, updateCheckAllowed]);

  useEffect(() => {
    if (!initialInfo && updateCheckAllowed) {
      fetchVersion();
    }
  }, [fetchVersion, initialInfo, updateCheckAllowed]);

  const handleUpgrade = useCallback(async () => {
    if (!info?.has_update || !updateApplyAllowed) return;
    setUpgrading(true);
    setSteps([]);
    setError(null);
  ...
  }, [i18n.language, info, t, updateApplyAllowed]);
        ...
        >
          ...
          <div className="flex items-center gap-2 px-4 pb-4">
            <button
              ...
              disabled={checking || !updateCheckAllowed}
            ...
            {info?.has_update && info.update_allowed !== false && updateApplyAllowed && (
              <button
