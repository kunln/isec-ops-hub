...
function renderOnboarding(props: Partial<React.ComponentProps<typeof OnboardingModal>> = {}) {
  return render(
    <MemoryRouter>
      <OnboardingModal onClose={vi.fn()} {...props} />
    </MemoryRouter>,
  );
}
...
describe('OnboardingModal', () => {
  ...
  it('does not probe ThreatBook service credentials before optional section is visible', async () => {
    renderOnboarding({ vendorOnboardingAllowed: true });
  ...
  it('hides ThreatBook onboarding by default when vendor policy is disabled', async () => {
    renderOnboarding();

    await screen.findByRole('button', { name: 'onboarding.bootstrap.savePrimary' });

    expect(screen.getByText('onboarding.bootstrap.welcomeHint')).toBeInTheDocument();
    ...
    expect(screen.queryByRole('option', { name: 'onboarding.bootstrap.providerThreatBookCn' })).not.toBeInTheDocument();
    expect(screen.queryByRole('option', { name: 'onboarding.bootstrap.providerThreatBookGlobal' })).not.toBeInTheDocument();
