...
interface OnboardingModalProps {
  onClose: () => void;
  vendorOnboardingAllowed?: boolean;
  productName?: string;
}
...
function isThreatBookProviderId(providerId: string | undefined | null): boolean {
  return THREATBOOK_PROVIDER_IDS.includes(providerId as any);
}

export default function OnboardingModal({
  onClose,
  vendorOnboardingAllowed = false,
  productName,
  ...
  const primaryProviders = useMemo(() => {
    const filtered = catalog.filter((provider) => {
      const providerIsThreatBook = isThreatBookProviderId(provider.id);
      if (providerIsThreatBook) {
        return vendorOnboardingAllowed || provider.id === resolvedDefaultModel?.providerId;
      }
      return (
    ...
    const preferredOrder = vendorOnboardingAllowed ? ['threatbook-cn-llm', 'threatbook-io-llm'] : [];
  ...
  }, [catalog, resolvedDefaultModel?.providerId, vendorOnboardingAllowed]);
  ...
  const primaryProviderIsThreatBook = useMemo(
    () => isThreatBookProviderId(primaryProviderId),
    [primaryProviderId]
  );
  ...
  const primaryResolvedProviderIsThreatBook = useMemo(
    () => isThreatBookProviderId(primaryResolvedProviderId),
    [primaryResolvedProviderId]
  );

  const getProviderLabel = (provider: CatalogProvider) => {
    if (vendorOnboardingAllowed && provider.id === 'threatbook-cn-llm') return t('onboarding.bootstrap.providerThreatBookCn');
    if (vendorOnboardingAllowed && provider.id === 'threatbook-io-llm') return t('onboarding.bootstrap.providerThreatBookGlobal');
  ...
  const primaryVendorSelectionDisabled = primaryProviderIsThreatBook && !vendorOnboardingAllowed;
  const canSavePrimary = primaryProviderIsThreatBook
    ? !primaryVendorSelectionDisabled && Boolean(primaryApiKey.trim())
    : Boolean(primaryProviderId && primaryModelId && primaryApiKey.trim());

  const canSaveOptionalThreatBook = vendorOnboardingAllowed
    && primaryConfigured
    && !primaryProviderIsThreatBook
    && Boolean(optionalThreatBookApiKey.trim());
  const canStart = hasLLM === true || primaryConfigured;
  const showOptionalThreatBookSection = vendorOnboardingAllowed && !primaryProviderIsThreatBook && primaryConfigured;
  ...
  const handleSavePrimary = async () => {
    if (primaryProviderIsThreatBook && !vendorOnboardingAllowed) {
      setPrimaryStatus({
        tone: 'error',
        message: t('onboarding.bootstrap.vendorProviderDisabled'),
      });
      return;
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] mx-4 overflow-hidden flex flex-col">
        <div className="relative px-6 pt-5 pb-4">
          ...
          <h2 className="text-lg font-bold text-gray-900 pr-8">
            {t('onboarding.title', { productName: productName || t('onboarding.productFallback') })}
          </h2>
        </div>
            ...
            <div className="px-4 py-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-amber-800">
                  {t('onboarding.bootstrap.welcomeTitle', { productName: productName || t('onboarding.productFallback') })}
                </span>
              </div>
              <p className="mt-2 text-sm text-amber-800/90 leading-relaxed">
                {t('onboarding.bootstrap.welcomeIntro', { productName: productName || t('onboarding.productFallback') })}
              </p>
              <p className="mt-2 text-xs text-amber-700/80 leading-relaxed">
                {vendorOnboardingAllowed
                  ? t('onboarding.bootstrap.vendorWelcomeHint')
          ...
          <div className="rounded-xl border border-gray-200 p-4 space-y-4">
            ...
            {!isLoading && !primarySectionCollapsed && !showPrimaryConfiguredDetails && (
              <>
                ...
                {primaryProviderIsThreatBook && vendorOnboardingAllowed && (
                ...
                {primaryProviderIsThreatBook && !vendorOnboardingAllowed && (
                  <p className="text-[11px] text-amber-700">
                    {t('onboarding.bootstrap.vendorProviderDisabled')}
                  </p>
                )}
                ...
                <div className="flex gap-2">
                  ...
                  {!primaryVendorSelectionDisabled && (
                    <>
                      <input
                        type="password"
                        value={primaryApiKey}
                  ...
                  {primaryVendorSelectionDisabled && !primaryConfigured && (
                    <button
                      type="button"
                      disabled
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-600 opacity-40 cursor-not-allowed text-white text-sm font-medium"
                    >
                      {t('onboarding.bootstrap.savePrimary')}
                    </button>
                  )}
                </div>
