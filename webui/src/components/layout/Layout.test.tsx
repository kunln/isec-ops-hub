...
describe('Layout onboarding entry', () => {
  beforeEach(() => {
    ...
    commercialAPI.getConnectivity.mockResolvedValue({
      data: {
    ...
    commercialAPI.getUpdatePolicy.mockResolvedValue({
      data: {
  ...
  it('does not build whats-new notifications when notification policy disables them', async () => {
    localStorage.setItem('flocks_onboarding_dismissed', 'true');
    commercialAPI.getNotificationPolicy.mockResolvedValue({
      data: {
        local_notifications_enabled: true,
