...
@pytest.mark.asyncio
async def test_daemon_retries_stale_session_on_same_target_before_fallback(monkeypatch, tmp_path) -> None:
    daemon = Daemon()
    daemon.session = "session-1"
    daemon.target_id = "target-1"
    monkeypatch.setattr("flocks.browser.daemon.LOG", str(tmp_path / "bu.log"))
