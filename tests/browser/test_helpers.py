...
def test_http_get_rejects_before_urlopen_when_outbound_disabled(monkeypatch) -> None:
    calls = []
    ...
    urlopen = MagicMock()
    monkeypatch.setattr(helpers.urllib.request, "urlopen", urlopen)
    ...
    with pytest.raises(commercial_policy.CommercialPolicyError, match="blocked by commercial policy"):
        helpers.http_get("https://example.com/")
