...
def test_tdp_log_search_schema_documents_default_sql_without_requiring_it():
    tool = _load_tool("tdp_log_search.yaml")

    schema = tool.info.get_schema()
    assert "sql" not in schema.required
    assert schema.properties["sql"]["default"] == "threat.level = 'attack'"
