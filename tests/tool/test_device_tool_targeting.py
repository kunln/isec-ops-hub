...
@pytest.mark.asyncio
async def test_device_tool_policy_uses_resolved_private_device_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    activated_ids = []
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value={
            "id": "dev-1",
            "fields": {"base_url": "https://192.168.31.182:443"},
        }),
    )
    ...
    monkeypatch.setattr("flocks.commercial.policy.ensure_outbound_allowed", fake_ensure_outbound_allowed)
    monkeypatch.setattr("flocks.tool.credential_context.activate_device_credentials", _activate)
    ...
    assert result.success is True
    assert activated_ids == ["dev-1"]
    assert captured_policy["url"] == "https://192.168.31.182:443"
    assert captured_policy["allow_private_network"] is True
