...
def test_mingyu_apt_device_is_listed_in_hub_catalog():
    entries = list_catalog(plugin_type="device", q="明御")
    match = next((entry for entry in entries if entry.id == "dbappsecurity_mingyu_apt_v2_0_r77"), None)

    assert match is not None
    assert match.type == "device"
    assert match.name == "明御 APT 攻击预警平台"
    assert "ndr" in match.tags
    assert match.manifestPath == "plugins/tools/device/dbappsecurity_mingyu_apt_v2_0_r77/manifest.json"


def test_mingyu_edr_device_is_listed_in_hub_catalog():
    entries = list_catalog(plugin_type="device", q="明御 EDR")
    match = next((entry for entry in entries if entry.id == "dbappsecurity_mingyu_edr_v2_0_17"), None)

    assert match is not None
    assert match.type == "device"
    assert match.name == "明御主机安全及管理系统 EDR"
    assert "edr" in match.tags
    assert "asset-discovery" in match.useCases
    assert match.manifestPath == "plugins/tools/device/dbappsecurity_mingyu_edr_v2_0_17/manifest.json"


def test_das_gateway_device_is_listed_in_hub_catalog():
    entries = list_catalog(plugin_type="device", q="DAS-Gateway")
    match = next((entry for entry in entries if entry.id == "dbappsecurity_das_gateway_v3_0_6_0r"), None)

    assert match is not None
    assert match.type == "device"
    assert match.name == "明御安全网关 DAS-Gateway（上网行为管理）"
    assert "network" in match.tags
    assert "log-analysis" in match.useCases
    assert match.manifestPath == "plugins/tools/device/dbappsecurity_das_gateway_v3_0_6_0r/manifest.json"


def test_mingjian_vuln_scanner_device_is_listed_in_hub_catalog():
    entries = list_catalog(plugin_type="device", q="明鉴")
    match = next((entry for entry in entries if entry.id == "dbappsecurity_mingjian_vuln_scanner_v5_0"), None)

    assert match is not None
    assert match.type == "device"
    assert match.name == "明鉴漏洞扫描系统"
    assert "vulnerability" in match.tags
    assert "vulnerability-management" in match.useCases
    assert match.manifestPath == "plugins/tools/device/dbappsecurity_mingjian_vuln_scanner_v5_0/manifest.json"
