"""Tests for Manual Sync Preview skeleton."""
...
async def seed_fake_profile(*, instance_id: str = "intinst_fake", sync_profile_id: str = "syncprof_fake", capability: str = "alert.search"):
...
@pytest.mark.asyncio
async def test_preview_sync_profile_run_success_records_run_and_preserves_profile(client: AsyncClient) -> None:
    ...
    result = await preview_sync_profile_run(
        ManualSyncPreviewRequest(sync_profile_id=profile.sync_profile_id, requested_by="tester", params_override={"limit": 2}, dry_run=False, preview_only=False),
    ...
    assert result.fetched_count == result.mapped_count == result.preview_count == 1
    assert result.request_summary["params"]["limit"] == 2
    assert result.dispatch_summary["preview_only"] is True
    assert result.dispatch_summary["create_analysis_cases"] == 0
    ...
    assert run["run_type"] == "sync_profile_preview"
...
@pytest.mark.asyncio
async def test_api_post_preview_success(client: AsyncClient) -> None:
    _, profile = await seed_fake_profile(sync_profile_id="syncprof_api")
    response = await client.post(
        "/api/security/integrations/sync-engine/preview",
        json={"sync_profile_id": profile.sync_profile_id, "params_override": {"limit": 3}, "dry_run": False, "preview_only": False},
    )
...
async def test_unknown_sync_profile_returns_404(client: AsyncClient) -> None:
    response = await client.post("/api/security/integrations/sync-engine/preview", json={"sync_profile_id": "missing"})
...
async def test_missing_instance_returns_400(client: AsyncClient) -> None:
    ...
    response = await client.post("/api/security/integrations/sync-engine/preview", json={"sync_profile_id": "syncprof_orphan"})
    assert response.status_code == 400


@pytest.mark.asyncio
async def test_missing_adapter_and_unsupported_capability_return_400(client: AsyncClient) -> None:
    _, profile = await seed_fake_profile(sync_profile_id="syncprof_no_adapter", capability="missing.search")
    response = await client.post("/api/security/integrations/sync-engine/preview", json={"sync_profile_id": profile.sync_profile_id})
    assert response.status_code == 400
...
async def test_params_override_secret_like_key_or_value_returns_400(client: AsyncClient, override: dict[str, object]) -> None:
    ...
    response = await client.post(
        "/api/security/integrations/sync-engine/preview",
        json={"sync_profile_id": profile.sync_profile_id, "params_override": override},
    )
    assert response.status_code == 400
...
@pytest.mark.asyncio
async def test_no_connector_http_credential_or_security_side_effects(client: AsyncClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("flocks.security.evidence_ingestion.ingest_external_events", forbidden)
    ...
    body = response.json()
    assert body["status"] == "previewed"
    assert body["safety_summary"]["credentials_read"] is False
    assert body["safety_summary"]["secret_ref_resolved"] is False
