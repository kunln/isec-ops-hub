...
class TestCLIIntegration:
    ...
    def test_cli_help_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "flocks.cli.main", "--help"],
            capture_output=True,
            text=True,
            timeout=10
        )
    ...
    def test_cli_version_command(self):
        """测试 CLI version 命令"""
        result = subprocess.run(
            [sys.executable, "-m", "flocks.cli.main", "--version"],
            capture_output=True,
            text=True,
            timeout=10
        )
...
class TestServerAPI:
    ...
    async def test_create_session_via_routes(self):
        ...
        from flocks.server.routes.session import SessionCreateRequest, create_session
        
        http_request = SimpleNamespace(
            state=SimpleNamespace(
                auth_user=AuthUser(id="usr_test", username="tester", role="admin")
            )
        )
        ...
        response = await create_session(http_request, request)
