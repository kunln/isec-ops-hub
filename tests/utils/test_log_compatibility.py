...
class TestLoggerCompatibility:
    ...
    def test_time_diff_calculation(self):
        ...
        import io
        old_stderr = Log._writer
        old_last_time = Log._last_time
        Log._writer = io.StringIO()
        Log._last_time = 100_000
        
        try:
            with patch("time.time", side_effect=[100.0, 100.01]):
                logger.info("first")
                logger.info("second")
...
class TestLogInitialization:
    ...
    async def test_init_creates_log_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            os.environ["FLOCKS_LOG_DIR"] = str(Path(tmpdir) / ".flocks" / "logs")
            ...
            try:
                ...
                os.environ.pop("FLOCKS_LOG_DIR", None)
                if Log._writer:
                    Log._writer.close()
                    Log._writer = None
