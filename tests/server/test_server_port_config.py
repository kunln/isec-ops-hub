...
def _bind_or_skip(sock: socket.socket, address: tuple[str, int]) -> None:
    """Bind a test socket, skipping when the sandbox forbids local binds."""
    try:
        sock.bind(address)
...
class TestPortAvailability:
    ...
    def test_port_is_not_available_when_in_use(self):
        ...
        try:
            _bind_or_skip(sock, ("127.0.0.1", test_port))
            sock.listen(1)
...
class TestPortConfigurationEdgeCases:
    ...
    def test_port_zero_dynamic_allocation(self):
        ...
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        _bind_or_skip(sock, ("127.0.0.1", 0))
...
class TestMultipleServerInstances:
    ...
    def test_two_servers_same_port_fails(self):
        ...
        _bind_or_skip(sock1, ("127.0.0.1", port))
        sock1.listen(1)
    ...
    def test_two_servers_different_ports_succeeds(self):
        ...
        try:
            _bind_or_skip(sock1, ("127.0.0.1", port1))
            sock1.listen(1)

            _bind_or_skip(sock2, ("127.0.0.1", port2))
            sock2.listen(1)
