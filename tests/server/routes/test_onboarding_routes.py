...
async def _allow_threatbook_onboarding() -> None:
    from flocks.commercial.store import default_store

    await default_store.update_connectivity({"outbound_enabled": True})
    await default_store.update_notification_policy({"vendor_notifications_enabled": True})
...
class TestOnboardingValidateRoutes:
    @pytest.mark.asyncio
    async def test_validate_rejects_threatbook_when_vendor_policy_disabled(
        self, client, monkeypatch: pytest.MonkeyPatch
    ):
        called = False
        ...
        assert "厂商引导已被商业化通知策略禁用" in resp.text
        assert called is False

    @pytest.mark.asyncio
    async def test_validate_rejects_threatbook_when_outbound_disabled(
        self, client, monkeypatch: pytest.MonkeyPatch
    ):
        from flocks.commercial.store import default_store
        ...
        assert resp.status_code == 403, resp.text
        assert "ThreatBook onboarding is disabled by commercial connectivity policy" in resp.text
        assert called is False
    ...
    async def test_validate_returns_region_mismatch_when_other_region_probe_succeeds(
        ...
        await _allow_threatbook_onboarding()
...
class TestOnboardingApplyRoutes:
    ...
    @pytest.mark.asyncio
    async def test_apply_rejects_threatbook_when_outbound_disabled_before_validation(
        self, client, monkeypatch: pytest.MonkeyPatch
    ):
        from flocks.commercial.store import default_store
