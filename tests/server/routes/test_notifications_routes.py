...
async def _enable_builtin_benefit_notifications() -> None:
    from flocks.commercial.models import NotificationPolicyUpdate
    from flocks.commercial.store import default_store

    await default_store.update_notification_policy(
        NotificationPolicyUpdate(
            built_in_notifications_enabled=True,
...
async def test_active_notifications_and_dismiss_forever(client: AsyncClient):
    await _enable_builtin_benefit_notifications()
...
@pytest.mark.asyncio
async def test_benefit_notifications_disabled_by_default():
    items = await NotificationService.list_active(
        user_id="user-a",
        locale="zh-CN",
    )
...
@pytest.mark.asyncio
async def test_whats_new_notifications_disabled_filters_config_notifications(monkeypatch):
    from flocks.commercial.models import NotificationPolicyUpdate
    from flocks.commercial.store import default_store
    from flocks.notifications import service as notification_service
    ...
    assert "whats-new-2099.01.01" not in {item.id for item in items}
