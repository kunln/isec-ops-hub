...
class TestDeviceTestEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_probe_marks_private_device_targets_as_allowed_network(
        ...
        monkeypatch.setattr(
            "flocks.commercial.policy.ensure_outbound_allowed",
            fake_ensure_outbound_allowed,
        )
        monkeypatch.setattr(device_routes.httpx, "AsyncClient", FakeAsyncClient)

        result = await device_routes._probe("https://192.168.31.182:443", verify_ssl=False)

        assert result.success is True
        assert captured["policy"]["url"] == "https://192.168.31.182:443"
        assert captured["policy"]["allow_private_network"] is True
        assert captured["url"] == "https://192.168.31.182:443"
