...
def _write_builtin_workflow(root: Path, workflow_id: str = "builtin-workflow") -> None:
    workflow_dir = root / workflow_id
    workflow_dir.mkdir(parents=True, exist_ok=True)
    workflow_json = {
        **_WORKFLOW_JSON,
        "name": "Built-in Workflow",
        "metadata": {"sampleInputs": {"keyword": "demo"}},
    }
    meta = {
        "name": "Built-in Workflow",
...
class TestWorkflowRoutes:
    ...
    @pytest.mark.asyncio
    async def test_clone_builtin_workflow_creates_global_custom_copy(
        self,
        client: AsyncClient,
        isolated_workflow_filesystem,
    ):
        """POST /api/workflow/{id}/clone copies a project template into global custom storage."""
        project_root = isolated_workflow_filesystem["project_root"]
        global_root = isolated_workflow_filesystem["global_root"]
        _write_builtin_workflow(project_root, "builtin-alert-triage")

        resp = await client.post(
            "/api/workflow/builtin-alert-triage/clone",
            json={"name": "My Alert Triage", "createdBy": "tester"},
        )

        assert resp.status_code == status.HTTP_201_CREATED, resp.text
        data = resp.json()
        assert data["id"] != "builtin-alert-triage"
        assert data["name"] == "My Alert Triage"
        assert data["source"] == "global"
        assert data["status"] == "active"
        assert data["workflowJson"]["metadata"]["sourceWorkflowId"] == "builtin-alert-triage"
        assert (global_root / data["id"] / "workflow.json").is_file()
        assert not (project_root / data["id"] / "workflow.json").exists()

    @pytest.mark.asyncio
    async def test_builtin_workflow_is_read_only(
        self,
        client: AsyncClient,
        isolated_workflow_filesystem,
    ):
        """Project workflows are built-in templates and cannot be mutated directly."""
        project_root = isolated_workflow_filesystem["project_root"]
        _write_builtin_workflow(project_root, "builtin-readonly")

        update_resp = await client.put(
            "/api/workflow/builtin-readonly",
            json={"name": "mutated"},
        )
        assert update_resp.status_code == status.HTTP_403_FORBIDDEN

        sample_resp = await client.post(
            "/api/workflow/builtin-readonly/sample-inputs",
            json={"sampleInputs": {"keyword": "changed"}},
        )
        assert sample_resp.status_code == status.HTTP_403_FORBIDDEN

        delete_resp = await client.delete("/api/workflow/builtin-readonly")
        assert delete_resp.status_code == status.HTTP_403_FORBIDDEN
        assert (project_root / "builtin-readonly" / "workflow.json").is_file()
