...
class TestMcpRoutes:
    ...
    @pytest.mark.asyncio
    async def test_catalog_install_enabled_persists_config_when_connect_fails(
        self, client: AsyncClient, monkeypatch: pytest.MonkeyPatch
    ):
        stored_configs: dict[str, dict] = {}
        attempted_connects: list[str] = []
        ...
        class CatalogEntryStub:
            ...
            def to_mcp_config(self, env_overrides=None, args=None):
                return {
                    ...
                    "transport": "http",
                    "headers": {"X-API-KEY": "{secret:asiainfo_threat_intel_mcp_key}"},
                }

        class CatalogStub:
            def get_entry(self, server_id: str):
                if server_id == "asiainfo_threat_intel_mcp":
                    return CatalogEntryStub()
                return None
        ...
        async def fake_status() -> dict[str, McpStatusInfo]:
            return {
                "asiainfo_threat_intel_mcp": McpStatusInfo(
                    status=McpStatus.FAILED,
                    error="HTTP 403 from https://llmsec.asiainfo-sec.com:8443/vulnquery/mcp",
                )
            }
        ...
        resp = await client.post(
            "/api/mcp/catalog/install",
            json={"server_id": "asiainfo_threat_intel_mcp", "enabled": True},
        )
        ...
        assert "HTTP 403" in data["connect_error"]
        assert stored_configs["asiainfo_threat_intel_mcp"]["enabled"] is True
        assert stored_configs["asiainfo_threat_intel_mcp"]["url"] == (
            "https://llmsec.asiainfo-sec.com:8443/vulnquery/mcp"
        )
        assert attempted_connects == ["asiainfo_threat_intel_mcp"]
