...
def isolated_env(tmp_path, monkeypatch):
    ...
    ``HOME`` is also redirected so :func:`_api_plugin_roots` (which walks
    ``Path.home() / .flocks/plugins/tools/api``) doesn't pick up the
    developer's real installs and break discovery / migration assertions.
    """
    from flocks.config.config import Config

    home = tmp_path / "home"
    project_root = tmp_path / "project"
    user_config = home / ".flocks" / "config"
    ...
    monkeypatch.setenv("FLOCKS_DATA_DIR", str(home / ".flocks" / "data"))
    monkeypatch.delenv("FLOCKS_CONFIG", raising=False)
    monkeypatch.delenv("FLOCKS_CONFIG_CONTENT", raising=False)
...
class TestConfigWriterFallback:
    ...
    def test_set_api_service_warns_when_writing_shadowed_legacy(
        self, isolated_env, api_root, capsys, monkeypatch,
    ):
        ...
        monkeypatch.setattr(Log, "_writer", None)
        monkeypatch.setattr(Log, "_level", LogLevel.INFO)
