...
def _use_fake_observer(monkeypatch: pytest.MonkeyPatch) -> None:
    watchdog_observers = pytest.importorskip("watchdog.observers")
    monkeypatch.setattr(watchdog_observers, "Observer", _FakeObserver)


def test_watcher_start_stop_no_crash(tmp_path, monkeypatch):
    """SkillFileWatcher can start and stop without errors."""
    from flocks.skill.skill import SkillFileWatcher

    _use_fake_observer(monkeypatch)
...
def test_start_stop_watcher_class_methods(tmp_path, monkeypatch):
