...
class TestCatalogEntry:
    ...
    def test_to_mcp_config_remote_url_env_override(self):
        entry = self._make_entry(
            transport="remote",
            remote=RemoteConfigSpec(
                url="",
                ...
                transport="http",
                headers={"X-API-KEY": "{secret:remote_mcp_key}"},
            ),
        )
        config = entry.to_mcp_config({"REMOTE_MCP_URL": "https://example.com/mcp"})

        assert config["type"] == "remote"
        assert config["url"] == "https://example.com/mcp"
        assert config["transport"] == "http"
        assert config["headers"]["X-API-KEY"] == "{secret:remote_mcp_key}"
...
class TestAsiaInfoThreatIntelMcpCatalog:
    """Verify the AsiaInfo Security vulnerability intelligence MCP entry."""

    @pytest.fixture()
    def entry(self):
        catalog = McpCatalog.get()
        entry = catalog.get_entry("asiainfo_threat_intel_mcp")
        assert entry is not None
        return entry

    def test_entry_is_searchable_by_vendor_and_vulnerability_terms(self, entry):
        catalog = McpCatalog.get()

        assert entry in catalog.search(query="AsiaInfo")
        assert entry in catalog.search(query="亚信")
        assert entry in catalog.search(query="query_vulnerability_by_id")
        assert entry in catalog.search(category="vulnerability")

    def test_entry_requires_url_and_header_key(self, entry):
        required = entry.required_env_vars

        assert entry.transport == "remote"
        assert entry.category == "vulnerability"
        assert required["ASIAINFO_THREAT_INTEL_MCP_URL"].secret is False
        assert required["ASIAINFO_THREAT_INTEL_MCP_KEY"].secret is True

    def test_entry_generates_streamable_http_header_config(self, entry):
        config = entry.to_mcp_config(
