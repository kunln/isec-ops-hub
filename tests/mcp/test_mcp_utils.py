...
class TestNormalizeMcpConfig:
    ...
    def test_project_asiainfo_mcp_config_uses_header_secret_and_streamable_http(self):
        config_path = (
            ...
            / "tools"
            / "mcp"
            / "asiainfo_threat_intel_mcp.yaml"
        )
        raw_text = config_path.read_text(encoding="utf-8")
        config = yaml.safe_load(raw_text)
        normalized = normalize_mcp_config(config)

        assert config["name"] == "asiainfo_threat_intel_mcp"
        assert config["url"] == "{secret:asiainfo_threat_intel_mcp_url}"
        assert config["headers"]["X-API-KEY"] == "{secret:asiainfo_threat_intel_mcp_key}"
        assert normalized["transport"] == "http"
        assert raw_text.count("{secret:") == 2
