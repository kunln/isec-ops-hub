...
**list**
  Compatibility alias for listing locally available skills.
  Prefer run_slash_command(command="skills") for UI-facing local skill views.
...
_ALLOWED_SUBCOMMANDS = frozenset(
    ["list", "find", "install", "status", "install-deps", "remove"]
)
_SUBCOMMAND_ENUM = ["list", "find", "install", "status", "install-deps", "remove"]

# Read-only registry / discovery — no shell side effects; skip bash permission gate.
_READ_ONLY_SUBCOMMANDS = frozenset({"list", "find", "status"})
