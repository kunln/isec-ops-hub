...
def _create_nested_tool_context(ctx: ToolContext) -> ToolContext:
    ...
    """
    return ToolContext(
        session_id=getattr(ctx, "session_id", None) or "workflow",
        message_id=getattr(ctx, "message_id", None) or "workflow",
        agent=getattr(ctx, "agent", None) or "workflow",
        call_id=getattr(ctx, "call_id", None),
        extra=dict(getattr(ctx, "extra", None) or {}),
        abort_event=getattr(ctx, "abort", None),
        permission_callback=getattr(ctx, "_permission_callback", None),
        metadata_callback=None,
        event_publish_callback=getattr(ctx, "event_publish_callback", None),
    )
