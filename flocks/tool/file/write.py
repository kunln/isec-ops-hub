...
async def _resolve_sandbox_file_path(ctx: ToolContext, filePath: str) -> tuple[str | None, str | None, dict | None]:
    sandbox = ctx.extra.get("sandbox") if ctx.extra else None
    sandbox_payload = sandbox if isinstance(sandbox, dict) else None
    try:
        resolution = await resolve_tool_path(ctx, filePath)
    except ValueError as exc:
        return None, str(exc), sandbox_payload
    sandbox_root = sandbox_payload.get("workspace_dir") if sandbox_payload else None
    if sandbox_root:
        expanded = os.path.expanduser(str(filePath).strip())
        if os.path.isabs(expanded):
            return os.path.normpath(os.path.abspath(expanded)), None, sandbox_payload
        return os.path.normpath(os.path.join(str(sandbox_root), expanded)), None, sandbox_payload
    return resolution.resolved_path, None, sandbox_payload
