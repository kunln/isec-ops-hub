    ...
    def _handle_rpc_request(self, *, msg: Dict[str, Any], token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if kind == "llm":
                ...
                ask_kwargs: Dict[str, Any] = {"temperature": temperature}
                if model is not None:
                    ask_kwargs["model"] = model
                if provider_id is not None:
                    ask_kwargs["provider_id"] = provider_id
                if timeout_s is not None:
                    ask_kwargs["timeout_s"] = timeout_s
                if "max_retries" in rpc:
                    ask_kwargs["max_retries"] = max_retries
                if "retry_delay_s" in rpc:
                    ask_kwargs["retry_delay_s"] = retry_delay_s
                output = get_lazy_llm().ask(prompt, **ask_kwargs)
                return {"type": "rpc_result", "token": token, "id": req_id, "ok": True, "output": output}
