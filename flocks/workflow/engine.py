...
def _run_async_policy_check(coro) -> None:
    import asyncio
    ...
    executor = getattr(_run_async_policy_check, "_executor", None)
    if executor is None:
        executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="wf-policy")
        setattr(_run_async_policy_check, "_executor", executor)
    executor.submit(lambda: asyncio.run(coro)).result()


def _ensure_workflow_http_outbound_allowed(url: str) -> None:
    ...
    _run_async_policy_check(_check())
...
class WorkflowEngine:
    ...
    def _execute_http_request_node(self, node: Node, inputs: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
        ...
        try:
            import httpx
            _ensure_workflow_http_outbound_allowed(url)
