...
from flocks.security.models import AnalysisFact, Incident
...
@router.post("/analysis-cases/from-alert/{alert_id}", dependencies=[Depends(require_capability("security.ops.write"))])
async def create_analysis_case_from_alert(alert_id: str):
    alert = await default_store.get_alert(alert_id)
    if alert is None:
        raise _not_found("Alert", alert_id)
    statement = f"Alert {alert.id} reported {alert.title}"
