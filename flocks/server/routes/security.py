...
from flocks.security.models import AnalysisFact, FactStrength, Incident
...
async def create_analysis_case_from_alert(alert_id: str):
    ...
    fact = AnalysisFact(
        ...
        source_references=[{"type": "alert", "id": alert.id}],
        raw_data={"severity": alert.severity, "source": alert.source},
    )
    return await default_store.create_analysis_case(
        AnalysisCaseCreate(
            title=f"告警研判：{alert.title}",
            alert_ids=[alert.id],
            asset_ids=[alert.asset_id] if alert.asset_id else [],
            facts=[fact],
            raw_data={"created_from": "alert", "alert_id": alert.id},
        )
    )
