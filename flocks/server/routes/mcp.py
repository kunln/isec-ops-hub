...
async def install_from_catalog(request: CatalogInstallRequest):
    ...
    try:
        ...
        if not request.skip_package_install:
            ...
            except commercial_policy.CommercialPolicyError as e:
                log.warn("mcp.catalog.preflight_denied", {"server_id": request.server_id, "error": str(e)})
        ...
        if request.enabled:
            # Persist first, then connect. A failed remote handshake is health
            # state, not a reason to lose the user's URL/credential setup.
            try:
                connected = await MCP.connect(request.server_id, config)
            except Exception as e:
                connected = False
                connect_error = str(e)
            if not connected and not connect_error:
                status = await MCP.status()
                server_status = status.get(request.server_id)
                connect_error = (
                    getattr(server_status, "error", None)
                    if server_status is not None
                    else None
                ) or f"Failed to connect to server: {request.server_id}"
            if connect_error:
                log.warn("mcp.catalog.connect_failed_after_save", {
        ...
        return {
            ...
            "message": (
                ...
                else f"Added {entry.name} to configuration, but connection failed: {connect_error}"
                if request.enabled and connect_error
