...
from flocks.commercial.policy import CommercialPolicyError
...
def _request_uses_threatbook(request: OnboardingValidateRequest) -> bool:
    return (
        request.use_threatbook_model
        or request.threatbook_services_only
        or bool((request.threatbook_api_key or "").strip())
    )


async def _ensure_threatbook_onboarding_allowed(request: OnboardingValidateRequest) -> None:
    if not _request_uses_threatbook(request):
        return

    notification_policy = await commercial_policy.get_notification_policy()
    if not notification_policy.vendor_notifications_enabled:
        raise HTTPException(
            status_code=403,
            detail="厂商引导已被商业化通知策略禁用",
        )

    preset = ONBOARDING_REGION_PRESETS[request.region]
    try:
        await commercial_policy.ensure_outbound_allowed(
            url=preset["activation_url"],
            purpose="ThreatBook onboarding",
        )
    except CommercialPolicyError as exc:
        raise HTTPException(
            status_code=403,
            detail=str(exc) or "主动外联已被商业化策略禁用",
        ) from exc
...
async def validate_onboarding(request: OnboardingValidateRequest) -> OnboardingValidateResponse:
    await _ensure_threatbook_onboarding_allowed(request)
