...
class WorkflowCloneRequest(BaseModel):
    """Request to clone an existing workflow into user-global custom storage."""
    model_config = ConfigDict(populate_by_name=True)
    ...
    status: Optional[Literal["draft", "active", "archived"]] = Field(
        None,
        description="New workflow status; defaults to the source workflow status",
    )
...
def _is_builtin_workflow(data: Dict[str, Any]) -> bool:
    """Project-level workflows are product templates and should be copied before editing."""
    return data.get("source") == "project"
...
async def _run_workflow_execution_task(
    *,
    ...
    def _on_step_start(_run_id, step_index, node, _inputs):
        ...
        if not step_history or step_history[-1].get("node_id") != node_id:
            step_history.append(pending_step)
        _write_progress({
    ...
    def _on_step_complete(step_result) -> None:
        ...
        if step_history and step_history[-1].get("node_id") == step_dict.get("node_id"):
            step_history[-1] = step_dict
        else:
...
@router.post("/workflow/{workflow_id}/clone", response_model=WorkflowResponse, status_code=status.HTTP_201_CREATED)
async def clone_workflow(workflow_id: str, req: WorkflowCloneRequest):
    """
    Clone workflow

    Copies any existing workflow into user-global custom storage. This is the
    supported path for modifying project-level built-in templates in the
    single-tenant product: copy first, edit the custom copy.
    """
    try:
        data = _read_workflow_from_fs(workflow_id)
        if not data:
        ...
        cloned_workflow_json = copy.deepcopy(data["workflowJson"])
        new_workflow_id = str(uuid.uuid4())
        now_ms = int(time.time() * 1000)
        source_name = data.get("name") or workflow_id
        new_name = req.name or f"{source_name} Copy"
        new_description = req.description if req.description is not None else data.get("description")
        new_status = req.status or data.get("status") or "draft"

        cloned_workflow_json["name"] = new_name
        metadata = cloned_workflow_json.get("metadata")
        if not isinstance(metadata, dict):
            metadata = {}
            cloned_workflow_json["metadata"] = metadata
        metadata["sourceWorkflowId"] = workflow_id
        metadata["sourceWorkflowName"] = source_name
        metadata["clonedAt"] = now_ms
        ...
        meta = {
            ...
            "category": data.get("category", "default"),
            "status": new_status,
            "createdBy": req.created_by or data.get("createdBy"),
            "createdAt": now_ms,
            "updatedAt": now_ms,
            "sourceWorkflowId": workflow_id,
        }

        _write_workflow_to_fs(new_workflow_id, cloned_workflow_json, meta, data.get("markdownContent"), global_store=True)

        stats = await _get_workflow_stats(new_workflow_id)
        response_data = {
            **meta,
            "workflowJson": cloned_workflow_json,
            "markdownContent": data.get("markdownContent"),
            "stats": stats,
            "source": "global",
        }

        log.info("workflow.cloned", {"id": new_workflow_id, "source_id": workflow_id, "name": new_name})
        await publish_event("workflow.created", {"id": new_workflow_id, "name": new_name, "sourceWorkflowId": workflow_id})
        return WorkflowResponse(**response_data)
    except HTTPException:
        raise
    except Exception as e:
        log.error("workflow.clone.error", {"id": workflow_id, "error": str(e)})
...
async def update_workflow(workflow_id: str, req: WorkflowUpdateRequest):
    ...
    try:
        ...
        if not data:
            raise HTTPException(status_code=404, detail=f"Workflow not found: {workflow_id}")
        if _is_builtin_workflow(data):
            raise HTTPException(status_code=403, detail="Built-in workflows are read-only. Clone the workflow before editing.")
...
async def delete_workflow(workflow_id: str):
    ...
    try:
        ...
        if not data:
            raise HTTPException(status_code=404, detail=f"Workflow not found: {workflow_id}")
        if _is_builtin_workflow(data):
            raise HTTPException(status_code=403, detail="Built-in workflows are read-only. Clone the workflow before deleting.")
...
async def save_sample_inputs(workflow_id: str, req: SampleInputsRequest):
    ...
    try:
        ...
        if not data:
            raise HTTPException(status_code=404, detail=f"Workflow not found: {workflow_id}")
        if _is_builtin_workflow(data):
            raise HTTPException(status_code=403, detail="Built-in workflows are read-only. Clone the workflow before saving sample inputs.")
