...
from flocks.updater import check_update, perform_update, detect_deploy_mode, get_current_version
...
async def check_version(
    ...
    return await check_update(locale=locale, enforce_commercial_policy=True)
...
async def apply_update(
    ...
    if not decision.allowed:
        raise HTTPException(
            ...
            detail=decision.reason or "Applying updates is disabled by commercial update policy",
        )
    ...
    if target_version:
        ...
        info = await check_update(locale=locale, enforce_commercial_policy=True)
        if info.error:
    ...
    async def _stream():
        gen = perform_update(
            ...
            locale=locale,
            enforce_commercial_policy=True,
        )
        try:
