...
async def lifespan(app: FastAPI):
    ...
    # Start connector sync scheduler. It is intentionally separate from the
    # general task queue because connector ingestion owns cursor and quality state.
    try:
        from flocks.security.connectors.scheduler import connector_sync_scheduler
        await _run_startup_phase(
            log,
            "connector_sync_scheduler.start",
            connector_sync_scheduler.start,
        )
        log.info("connector_sync_scheduler.started")
    except Exception as e:
        log.warning("connector_sync_scheduler.start.failed", {"error": str(e)})

    # Start credential expiry monitor so connector credentials warn before they
    # become sync blockers.
    try:
        from flocks.security.connectors.expiry_monitor import connector_credential_expiry_monitor_scheduler
        await _run_startup_phase(
            log,
            "connector_credential_expiry_monitor.start",
            connector_credential_expiry_monitor_scheduler.start,
        )
        log.info("connector_credential_expiry_monitor.started")
    except Exception as e:
        log.warning("connector_credential_expiry_monitor.start.failed", {"error": str(e)})
    ...
    try:
        ...
        log.info("connector_sync_scheduler.stopped")
    except Exception as e:
        log.warning("connector_sync_scheduler.stop.failed", {"error": str(e)})

    # Stop credential expiry monitor
    try:
        from flocks.security.connectors.expiry_monitor import connector_credential_expiry_monitor_scheduler
        await connector_credential_expiry_monitor_scheduler.stop()
        log.info("connector_credential_expiry_monitor.stopped")
    except Exception as e:
        log.warning("connector_credential_expiry_monitor.stop.failed", {"error": str(e)})
...
async def auth_guard_middleware(request: Request, call_next):
    ...
    try:
        ...
        from flocks.commercial.access_control import capability_for_api_request, require_capability_for_request

        capability = capability_for_api_request(request.url.path, request.method)
        if capability:
            await require_capability_for_request(
                request,
                capability,
                action="access_control.api",
...
app.include_router(commercial_admin_auth_router, prefix="/api/commercial-admin/auth", tags=["CommercialAdminAuth"])
