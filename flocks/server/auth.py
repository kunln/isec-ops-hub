...
# Public read-only endpoints. The method check happens in
# ``auth_middleware_exempt`` so adjacent write endpoints stay protected.
PUBLIC_GET_PATHS = frozenset({
    "/api/commercial/branding",
})
...
def auth_middleware_exempt(path: str, method: str | None = None) -> bool:
    """Return True if the path is public (no auth required)."""
    if method and method.upper() == "GET" and path in PUBLIC_GET_PATHS:
        return True
...
async def apply_auth_for_request(request: HTTPConnection):
    ...
    """
    if auth_middleware_exempt(request.url.path, getattr(request, "method", None)):
        token = set_current_auth_user(None)
        return None, token, None

    try:
        from flocks.server.commercial_admin_auth import (
            commercial_admin_cookie_allowed_for_path,
            commercial_admin_user_from_request,
        )

        if commercial_admin_cookie_allowed_for_path(request.url.path):
            commercial_admin = commercial_admin_user_from_request(request)  # type: ignore[arg-type]
            if commercial_admin is not None:
                request.state.auth_user = commercial_admin
                token = set_current_auth_user(commercial_admin)
                return None, token, commercial_admin
