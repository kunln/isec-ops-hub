...
from flocks.commercial.models import NotificationPolicy
...
NotificationKind = Literal["benefit", "whats_new", "announcement", "vendor"]
...
class NotificationService:
    ...
    @classmethod
    async def _load_notification_policy(cls) -> NotificationPolicy:
        try:
            return await commercial_policy.get_notification_policy()
        except Exception as exc:
            log.warn("notifications.policy.load_failed", {"error": str(exc)})
            return NotificationPolicy()

    @classmethod
    def _kind_enabled(cls, kind: NotificationKind, policy: NotificationPolicy) -> bool:
        if kind == "benefit":
            return policy.benefit_notifications_enabled
        if kind == "whats_new":
            return policy.whats_new_notifications_enabled
        if kind == "vendor":
            return policy.vendor_notifications_enabled
        if kind == "announcement":
            return policy.announcement_notifications_enabled
        return False
    ...
    async def list_active(
        ...
        *,
        ...
        notification_policy = await cls._load_notification_policy()
        config_notifications = (
            await cls._load_config_notifications()
            if notification_policy.local_notifications_enabled
            else []
        )
        ...
        notifications = [
            ...
            *(
                ...
                if notification_policy.built_in_notifications_enabled
                and notification.id not in config_ids
            ),
        ]
        ...
        for notification in sorted(
            ...
            if not cls._kind_enabled(notification.kind, notification_policy):
                continue
