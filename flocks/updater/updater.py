...
async def _fetch_github_release(
    ...
    *,
    enforce_commercial_policy: bool = False,
    ...
    if enforce_commercial_policy:
        await commercial_policy.ensure_outbound_allowed(url=url, purpose="update check")
...
async def _fetch_gitee_release(
    ...
    *,
    ...
    if enforce_commercial_policy:
        await commercial_policy.ensure_outbound_allowed(url=api_url, purpose="update check")
    params: dict[str, str] = {}
    if token:
...
async def _fetch_gitlab_release(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        if enforce_commercial_policy:
            await commercial_policy.ensure_outbound_allowed(url=releases_url, purpose="update check")
        ...
        if enforce_commercial_policy:
            await commercial_policy.ensure_outbound_allowed(url=tags_url, purpose="update check")
...
async def _fetch_release_from_source(
    ...
    if source == "github":
        return await _fetch_github_release(
            repo,
            token,
            base_url,
            enforce_commercial_policy=enforce_commercial_policy,
        )
    if source == "gitee":
        return await _fetch_gitee_release(
            gitee_repo or repo,
            gitee_token,
            enforce_commercial_policy=enforce_commercial_policy,
        )
    if source == "gitlab":
        return await _fetch_gitlab_release(
            repo,
            token,
            base_url,
            enforce_commercial_policy=enforce_commercial_policy,
...
async def _download_archive(
    ...
    *,
    ...
    if enforce_commercial_policy:
        await commercial_policy.ensure_outbound_allowed(url=url, purpose="update download")
...
async def check_update(
    *,
    ...
    if enforce_commercial_policy:
        decision = await commercial_policy.decide_update_check_allowed()
...
async def perform_update(
    ...
    *,
    ...
    ucfg = await _get_updater_config()
    if enforce_commercial_policy:
        decision = await commercial_policy.decide_update_apply_allowed()
        if not decision.allowed:
            yield UpdateProgress(
                stage="error",
                message=decision.reason or "Applying updates is disabled by commercial update policy",
                success=False,
            )
            return
    profile = _resolve_update_mirror_profile(
