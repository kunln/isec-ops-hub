...
async def _ensure_skill_outbound_allowed(*, url: str | None = None, purpose: str) -> None:
    await commercial_policy.ensure_outbound_allowed(
        url=url,
        purpose=purpose,
        require_initialized=False,
        require_url_for_allowed_hosts=url is None,
    )
...
class SkillInstaller:
    ...
    async def _install_from_clawhub(cls, name: str, scope: str) -> SkillInstallResult:
        ...
        try:
            await _ensure_skill_outbound_allowed(
                url=zip_url,
                purpose="skill registry download",
            )
    ...
    async def _download_github_dir(
        ...
        await _ensure_skill_outbound_allowed(
            url=api_url,
            purpose="skill GitHub directory download",
        )
        ...
        await _ensure_skill_outbound_allowed(
            url=skill_md_entry["download_url"],
            purpose="skill GitHub file download",
        )
    ...
    async def _execute_install_spec(
        ...
        try:
            await _ensure_skill_outbound_allowed(
                purpose=f"skill dependency install: {spec.kind}",
            )
