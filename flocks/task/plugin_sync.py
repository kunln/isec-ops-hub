...
async def upsert_task_specs(specs: Sequence[TaskSpec]) -> int:
    ...
    for spec in specs:
        ...
        if exec_mode == ExecutionMode.WORKFLOW and not spec.workflow_id:
            log.warn(
                "task.plugin.missing_workflow_id",
                {"dedup_key": spec.dedup_key, "execution_mode": spec.execution_mode},
            )
            continue
        ...
        if existing is not None:
            ...
            existing.workflow_id = spec.workflow_id
