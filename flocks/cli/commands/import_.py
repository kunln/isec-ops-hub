...
async def _import_session(file_or_url: str, project_id: Optional[str]):
    ...
    if is_url:
        ...
        try:
            ...
            opncd_url = f"https://opncd.ai/api/share/{slug}"
            flocks_url = f"https://flocks.ai/api/share/{slug}"
            async with httpx.AsyncClient() as client:
                # Try opncd.ai first
                await commercial_policy.ensure_outbound_allowed(
                    url=opncd_url,
                    purpose="session share import",
                    require_initialized=False,
                )
                response = await client.get(opncd_url)
                
                if response.status_code != 200:
                    # Try flocks.ai
                    await commercial_policy.ensure_outbound_allowed(
                        url=flocks_url,
                        purpose="session share import fallback",
                        require_initialized=False,
                    )
                    response = await client.get(flocks_url)
