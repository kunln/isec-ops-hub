...
def http_get(url: str, headers: dict | None = None, timeout: float = 20.0) -> str:
    ...
    try:
        ...
        asyncio.run(_check_outbound())
    else:
        from concurrent.futures import ThreadPoolExecutor

        with ThreadPoolExecutor(max_workers=1, thread_name_prefix="browser-http-policy") as executor:
            executor.submit(lambda: asyncio.run(_check_outbound())).result()
