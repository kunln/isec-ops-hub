...
def refresh_catalog_cache() -> None:
    """Clear filesystem-backed Hub catalog caches.

    The Hub catalog is mostly filesystem-backed, but the index and taxonomy
    JSON reads are cached for normal page loads. The explicit refresh route
    should therefore invalidate these caches before re-listing entries so
    newly added bundled packages appear without a process restart.
    """
