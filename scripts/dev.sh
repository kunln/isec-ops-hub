...
COMMERCIAL_ADMIN_FRONTEND_HOST="${COMMERCIAL_ADMIN_FRONTEND_HOST:-${COMMERCIAL_ADMIN_HOST:-127.0.0.1}}"
COMMERCIAL_ADMIN_FRONTEND_PORT="${COMMERCIAL_ADMIN_FRONTEND_PORT:-${COMMERCIAL_ADMIN_PORT:-51174}}"
...
Usage:
  ...
  ./scripts/dev.sh frontend     只启动前台前端
  ./scripts/dev.sh frontstage   只启动前台前端
  ./scripts/dev.sh commercial-admin  只启动商业化后台前端

Environment variables:
  BACKEND_HOST   默认 127.0.0.1
  BACKEND_PORT   默认 8000
  FRONTEND_HOST  默认 127.0.0.1
  FRONTEND_PORT  默认 8080
  COMMERCIAL_ADMIN_FRONTEND_HOST  默认 127.0.0.1
  COMMERCIAL_ADMIN_FRONTEND_PORT  默认 51174
EOF
}
...
cleanup() {
    ...
    if [ -n "${COMMERCIAL_ADMIN_PID}" ]; then
        echo -e "${YELLOW}\n🛑 停止商业化后台前端...${NC}"
        stop_process_tree "${COMMERCIAL_ADMIN_PID}" "商业化后台前端"
    fi
}
...
start_frontend() {
    echo -e "${GREEN}🎨 启动前台前端服务: http://${FRONTEND_HOST}:${FRONTEND_PORT}${NC}"
    cd "${PROJECT_ROOT}/webui"
    FLOCKS_API_PROXY_TARGET="${BACKEND_BASE_URL}" \
    FLOCKS_WEBUI_ENTRY=frontstage \
    FLOCKS_WEBUI_PORT="${FRONTEND_PORT}" \
    npm run dev:frontstage -- --host "${FRONTEND_HOST}" --port "${FRONTEND_PORT}"
}

start_commercial_admin_frontend() {
    echo -e "${GREEN}🏢 启动商业化后台前端服务: http://${COMMERCIAL_ADMIN_FRONTEND_HOST}:${COMMERCIAL_ADMIN_FRONTEND_PORT}${NC}"
    cd "${PROJECT_ROOT}/webui"
    FLOCKS_API_PROXY_TARGET="${BACKEND_BASE_URL}" \
    FLOCKS_WEBUI_ENTRY=commercial-admin \
    FLOCKS_WEBUI_PORT="${COMMERCIAL_ADMIN_FRONTEND_PORT}" \
    npm run dev:commercial-admin -- --host "${COMMERCIAL_ADMIN_FRONTEND_HOST}" --port "${COMMERCIAL_ADMIN_FRONTEND_PORT}"
}
...
start_all() {
    ...
    start_commercial_admin_frontend &
    COMMERCIAL_ADMIN_PID=$!
    echo -e "${YELLOW}商业化后台前端 PID: ${COMMERCIAL_ADMIN_PID}${NC}"
