...
The first Evidence Dispatcher skeleton previews lightweight Evidence Events produced by the Mapping Engine and may reuse the existing evidence ingestion pipeline only when explicitly requested; preview mode is read-only and creates no Alerts or Analysis Cases.
