...
To validate an existing local setup without installing dependencies:

```bash
make bootstrap-check
```

The default bootstrap installs Python dev dependencies with `uv sync --group dev` and WebUI dependencies with `npm ci`. TUI dependencies are optional and require Bun:
...
The stable pre-merge gate is:

```bash
make gate
```

It verifies the local environment, runs the scoped Python and WebUI lint/build checks, validates the bundled Hub, then runs the P0 acceptance baseline:

```bash
make acceptance
```

`make acceptance` is the minimum green baseline and runs `make bootstrap-check`, `make test`, `make test-webui`, and `make validate-security`.
...
- Cross-cutting changes: run `make gate`, or explain exactly which gate step could not be run and why.
