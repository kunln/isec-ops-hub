...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall flocks and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Flocks")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g flocks-ai",
      pnpm: "pnpm uninstall -g flocks-ai",
      bun: "bun remove -g flocks-ai",
      yarn: "yarn global remove flocks-ai",
      brew: "brew uninstall flocks",
      choco: "choco uninstall flocks",
      scoop: "scoop uninstall flocks",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall flocks -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Flocks!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# flocks") || content.includes(".flocks/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".flocks/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".flocks/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".flocks"))
    ) {
      continue
    }
