...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run flocks",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for flocks session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found flocks session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`flocks import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting flocks...")
        UI.println()

        // Launch flocks TUI with session ID if available
        const { spawn } = await import("child_process")
        const flocksArgs = sessionId ? ["-s", sessionId] : []
        ...
        await new Promise<void>((resolve, reject) => {
          flocksProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`flocks exited with code ${code}`))
          })
          flocksProcess.on("error", reject)
        })
      },
    })
  },
