    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: flocks mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in flocks.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        const authPath = path.join(Global.Path.config, "mcp-auth.json")
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .flocks/ subdirectory too)
  const candidates = [path.join(baseDir, "flocks.json"), path.join(baseDir, "flocks.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".flocks", "flocks.json"), path.join(baseDir, ".flocks", "flocks.jsonc"))
  }
  ...
  // Default to flocks.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., flocks x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "flocks-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
