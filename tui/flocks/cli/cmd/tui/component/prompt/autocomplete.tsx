...
export function Autocomplete(props: {
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    const tuiSlashEntries = command.slashes()
    const results: AutocompleteOption[] = [...tuiSlashEntries]

    // Names and aliases already provided by TUI (avoid duplicate /help, /clear, /compact, /skills)
    const tuiNames = new Set(
      tuiSlashEntries.flatMap((s) => {
        const name = s.display.trimEnd().replace(/^\//, "")
        const aliasNames = (s.aliases ?? []).map((a) => a.replace(/^\//, ""))
        return [name, ...aliasNames]
      }),
    )

    for (const serverCommand of sync.data.command) {
      if (tuiNames.has(serverCommand.name)) continue
      results.push({
